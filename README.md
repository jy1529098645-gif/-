...
浏览器打开 http://localhost:8501 。暗色现代仪表盘：
- **🎖️ 建仓/撤离作战卡**（高级研究·首位） — 把**全套回测验证过**的逻辑落成一张卡：今天该建多少仓(四档风险偏好·波动目标18/25/35/40%→建议暴露%，含 **🔥杠杆进取档** ≤1.5×)、回撤桶建仓证据(单票suggestive/池化验证级)、撤离触发(趋势死亡=崩盘保险口径)。证伪的逻辑(固定%止损/快速二元出场)一律不进。🔥杠杆档=趋势门+波动目标带杠杆，科技/半导体回测(2010-26)年化+22.6%(中性+14.4%·死拿+29.4%)、夏普1.12、回撤−27%——**放大beta非择时，强制显示杠杆/回撤代价**。[analysis/position_guidance.py](analysis/position_guidance.py)
- **🏆 最推荐买入·选股榜**（高级研究） — 多因子横截面综合评分(风险调整动量+趋势健康+相对强度，z-score+winsorize)→排名，**带 Alphalens 口径 IC/RankIC/ICIR + 分位价差 + 安慰剂验证**。诚实裁决：科技窄池实测 **RankIC≈安慰剂(无横截面预测力)**——故定位为**健康趋势票筛选器**(选趋势最健康的票用 v3 策略参与、回避破位的)，**非"top1会跑赢"的预测**。质量/价值因子需 PIT 付费数据故不入评分。[analysis/stock_ranking.py](analysis/stock_ranking.py)
- **🏠 概览** — 三铁律 + 数据源状态 + 术语表
...
- **自动留痕**：每天打开标的即静默记录当日指导（决策状态/建议仓位/历史常驻价/引擎预测胜率·按 标的+日期+周期 去重）。
...
## 📐 工具的 Alpha / Beta（大规模回测·诚实复现）

降偏差宽池(11赢家+10走平/跌)+ SPY 跨 2000 至今、含费、α 带 block bootstrap CI、过 deflated Sharpe：

- **入场择时(dip-from-high)**：N_eff 折算后超额**不显著**；OOS 跑赢持有比例 39%(<50%)——**择时本身无统计优势**。
- **快速/二元出场**(固定%移损/200线破位/MACD/分批快撤)：对同篮子持有**显著负α**、反拖累夏普；**出场越快夏普越差(单调)**。被工具明确排除。
- **连续波动目标定仓(v3)**：唯一对同篮子持有做出**显著正α(+3.7%, CI不跨0)**的杠杆；组合夏普 1.01(持有0.85)、回撤 −54%→−20%、**Deflated Sharpe 0.999**(扣10方案多重检验后仍稳健)。
- ⚠️ **诚实天花板**：v3 的正α**只在崩盘年兑现**(剔2000/01/02/08/22后α+1.0%不显著、26年里22年闭眼持有夏普更高)；**绝对收益上没有任何配置跑赢长牛持有**——减暴露必然让出复利。
- **结论**：工具不是高 alpha / 可一键下单的择时引擎。可证明的价值=**风险调整后更优 + 砍一半回撤的崩盘保险 + 校准预期**。回测脚本见 `scripts/{mega_backtest,portfolio_alpha_beta,v2_exit_backtest,exit_family_bakeoff,v3_oos_dsr}.py`。

> 📜 旧版曾写"系统α+8.1%/β0.75、风险叠加α+2.2%显著"——那用了更集中的赢家组合，偏乐观；本节为降偏差宽池 + 严格等权 + DSR 把关的诚实复现(overlay 自身 α 多数口径下不显著，价值在降β/降回撤)。
