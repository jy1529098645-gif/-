...
- **🎖️ 建仓/撤离作战卡**（高级研究·首位） — 把**全套回测验证过**的逻辑落成一张卡：今天该建多少仓(三档风险偏好·波动目标18/25/35%→建议暴露%)、回撤桶建仓证据(单票suggestive/池化验证级)、撤离触发(趋势死亡=崩盘保险口径)。证伪的逻辑(固定%止损/快速二元出场)一律不进。[analysis/position_guidance.py](analysis/position_guidance.py)
- **🏠 概览** — 三铁律 + 数据源状态 + 术语表
