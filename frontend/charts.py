"""前端交互图（plotly，明暗双主题：配色经 frontend/theme.py 的 C 代理动态解析）。
...
class _Palette:
    """按当前主题（暗/亮）动态解析的图表调色板：C["accent"] 始终取当前主题对应色。"""
    def __getitem__(self, key):
        return _theme.chart_tokens()[key]

    def get(self, key, default=None):
        return _theme.chart_tokens().get(key, default)
...
def _style(fig: go.Figure, height: int = 380, title: str = "") -> go.Figure:
    ...
    _spk = dict(showspikes=True, spikesnap="cursor", spikecolor=C["spike"],
                spikethickness=1, spikedash="dot")
    # 显式指定刻度字色=主题文字色：plotly_dark/white 模板会塞自己的 tickfont 覆盖 layout.font，
    # 不强制会导致浅色主题下刻度仍是浅灰、白底上看不见。
    _tf = dict(tickfont=dict(color=C["text"]))
    fig.update_xaxes(gridcolor=C["grid"], zerolinecolor=C["grid"], spikemode="across", **_tf, **_spk)
    fig.update_yaxes(gridcolor=C["grid"], zerolinecolor=C["grid"], **_tf, **_spk)
    return fig
...
def return_hist(returns, median=None, baseline_median=None, n=None, n_eff=None, title="") -> go.Figure:
    ...
    fig.add_vline(x=0, line=dict(color=C["faint"], width=1))
...
def event_study(study: dict, title="财报日历结构") -> go.Figure:
    ...
    fig.add_vline(x=0, line=dict(color=C["spike"], width=1.5, dash="dash"),
...
# TradingView 式时间区间按钮（按当前主题着色，故用函数而非模块常量）
def _range_buttons() -> dict:
    return dict(
        ...
        bgcolor=C["band"], activecolor=C["accent"],
        font=dict(color=C["text"], size=11), x=0, y=1.12,
    )
...
def _apply_tv(fig: go.Figure, height: int, title: str) -> go.Figure:
    ...
    fig.update_layout(
        template=_theme.plotly_template(), paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)",
    ...
    # tickfont 显式给主题文字色，避免模板的浅灰刻度在白底上看不见
    _tf = dict(tickfont=dict(color=C["text"]))
    fig.update_xaxes(gridcolor=C["grid"], showspikes=True, spikemode="across+marker", **_tf,
                     spikesnap="cursor", spikecolor=C["accent2"], spikethickness=1, spikedash="dot")
    fig.update_yaxes(gridcolor=C["grid"], showspikes=True, spikemode="across+marker", spikesnap="cursor", **_tf,
                     spikecolor=C["accent2"], spikethickness=1, spikedash="dot")
    return fig
...
def trade_map_candles(ohlcv: pd.DataFrame, trades: pd.DataFrame, title="进出场标记图", logy=True) -> go.Figure:
    ...
    vcol = np.where(up, C["vol_up"], C["vol_down"])
...
def volume_profile_bars(vp: dict, title="Volume Profile（日线近似）") -> go.Figure:
    ...
    colors = [C["accent"] if (va_lo <= c <= va_hi) else "rgba(124,92,252,0.35)" for c in vp["centers"]]
    fig.add_bar(x=vp["volumes"], y=vp["centers"], orientation="h", marker_color=colors, name="成交量")
    fig.add_hline(y=poc, line=dict(color=C["accent2"], width=2, dash="dash"),
...
def panorama_price_chart(ohlcv: pd.DataFrame, zones=None, vp: dict | None = None,
    ...
    def _hline(level, color, width, dash, label=None, side="right"):
        # 价位横线用「全宽 x 域」的 line shape（x0=0→x1=1，xref=x domain）：始终横贯整个绘图区、
        # 不随横向缩放/平移收缩或漂走，与下方钉在轴边缘的标签对齐。
        _lvls.append(float(level))
        fig.add_shape(type="line", xref="x domain", x0=0, x1=1, yref="y", y0=level, y1=level,
                      line=dict(color=color, width=width, dash=dash), layer="above", row=1, col=1)
        if label:
    ...
    def _band(y0, y1, fillcolor, label=None, lab_color=C["text"]):
        # 阴影带用全宽 rect shape（xref=x domain 0→1），同理横贯整图、缩放不变形。
        _lvls.extend([float(y0), float(y1)])
        fig.add_shape(type="rect", xref="x domain", x0=0, x1=1, yref="y", y0=y0, y1=y1,
                      fillcolor=fillcolor, line=dict(width=0), layer="below", row=1, col=1)
        if label:
    ...
    if show_best and best_entry and best_entry.get("has_zone"):
        if not best_endorsed:
            ...
            gcol = C["candle_up"]; gfill = "rgba(43,230,168,0.15)"; _blab = "🎯最佳入场区"
        else:
            gcol = C["gold"]; gfill = "rgba(255,209,102,0.15)"; _blab = "🎯最佳入场区"
    ...
    fig.add_trace(go.Candlestick(x=ohlcv.index, open=ohlcv["open"], high=ohlcv["high"], low=ohlcv["low"],
                  close=ohlcv["close"], increasing_line_color=C["candle_up"], decreasing_line_color=C["candle_down"],
    ...
    if show_zones and zones is not None and len(zones):
        ...
        for _, r in zz.iterrows():
            ...
            col = C["accent2"] if cur else "rgba(138,147,166,0.55)"
            _hline(float(r["price_high"]), col, 1.4 if cur else 1.0, "dot",
                   ("▶ " if cur else "") + str(r["zone"]), side="left")
    if show_vp and vp is not None:
        _hline(vp["poc"], C["amber"], 1.6, "dash", f"POC {vp['poc']:.1f}", side="right")
    if show_best and best_entry and best_entry.get("has_zone"):
        if not best_endorsed:
            gcol = "rgba(138,147,166,0.95)"; _alab = "📍历史常驻价"
        else:
            gcol = C["candle_up"] if best_entry.get("tier") == "稳健最佳入场区" else C["gold"]; _alab = "🎯历史常驻价"
    ...
    if show_rsi:
        ...
        fig.add_trace(go.Scatter(x=ohlcv.index, y=_rsi, mode="lines", name=f"RSI({int(rsi_window)})",
                                 line=dict(color=C["accent2"], width=1.4), showlegend=False,
...
def candle_with_levels(ohlcv: pd.DataFrame, vp: dict, title="近一年 K线 + 筹码分布", logy=False) -> go.Figure:
    ...
    fig.add_trace(go.Bar(x=ohlcv.index, y=ohlcv["volume"], marker_color=vcol, showlegend=False,
                  hovertemplate="量 %{y:.3s}<extra></extra>"), row=2, col=1)
    # POC/价值区用全宽 x 域 shape（xref=x domain 0→1）：横贯整图、缩放/平移不收缩漂移，与钉边标签对齐
    fig.add_shape(type="rect", xref="x domain", x0=0, x1=1, yref="y", y0=va_lo, y1=va_hi,
                  fillcolor=C["value"], line=dict(width=0), layer="below", row=1, col=1)
    fig.add_shape(type="line", xref="x domain", x0=0, x1=1, yref="y", y0=poc, y1=poc,
                  line=dict(color=C["accent2"], width=1.5, dash="dash"), layer="above", row=1, col=1)
...
def options_oi(snap: dict, title="期权未平仓（OI）按行权价") -> go.Figure:
    ...
    fig.add_bar(x=cg.index, y=cg.values, name="Call OI", marker_color=C["candle_up"], opacity=0.7)
    fig.add_bar(x=pg.index, y=pg.values, name="Put OI", marker_color=C["candle_down"], opacity=0.7)
    fig.add_vline(x=snap["spot"], line=dict(color=C["accent2"], width=2), annotation_text=f"现价 {snap['spot']:.1f}")
...
def signal_excess(df: pd.DataFrame, title="信号远期超额（vs 各股基准，95% CI）") -> go.Figure:
    ...
    fig.add_vline(x=0, line=dict(color=C["spike"], width=1))
...
def score_timeseries(df: pd.DataFrame, ticker="") -> go.Figure:
    ...
    fig.add_trace(go.Scatter(x=df.index, y=df["price"], line=dict(color=C["baseline"], width=1.2), name="价格"), row=1, col=1)
...
def event_timeline_chart(price: pd.Series, events: pd.DataFrame, title="事件时间线") -> go.Figure:
    ...
    fig = go.Figure()
    fig.add_scatter(x=price.index, y=price.values, mode="lines", line=dict(color=C["baseline"], width=1.2), name="价格", showlegend=False)
    pser = price.dropna()
    for typ, col in [("财报", C["candle_up"]), ("SEC", C["accent2"])]:
...
def entry_zone_bars(zones: pd.DataFrame, horizon: int, title="条件价位带 · 远期收益分布") -> go.Figure:
    ...
    fig.add_bar(y=labels, x=z["median"], orientation="h", marker_color=colors,
                error_x=dict(type="data", symmetric=False, array=hi, arrayminus=lo, color=C["spike"]),
...
def price_with_zones(price: pd.Series, zones: pd.DataFrame, lookback: int = 504,
    ...
    for _, r in zones.iterrows():
        lvl = r["price_high"]
        cur = r["is_current"]
        fig.add_hline(y=lvl, line=dict(color=C["win"] if cur else C["grid"],
    ...
    fig.add_hline(y=cur_px, line=dict(color=C["gold"], width=1.2),
                  annotation_text=f"现价 {cur_px:.0f}", annotation_position="left",
                  annotation_font=dict(size=10, color=C["gold"]))
...
def equity_compare(equity: pd.DataFrame, title="策略 vs 持有 · 净值(归一)") -> go.Figure:
    ...
    fig.add_hline(y=1.0, line=dict(color=C["faint"], width=1))
