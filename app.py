...
def page_panorama():
    ...
    try:
        ...
        if _is_holder:
            ...
            st.caption("📍 说明：上方『入场位』=现价下方**可执行回踩支撑**（MA/POC/前低/Fib）——回踩到此分批，进场更不易被套。"
                       "（入场对长期收益影响小：要的是进得稳、拿得住，不是抄到精确最低点。）")
        else:
            st.caption("📍 说明：当前是**飞刀/离场红灯**——不建新仓，故上方不给入场价。"
                       "想建仓请等站回200线 / 企稳 / 预警解除；已持仓者直接看下方『📌 已建仓怎么办』。")
    ...
    _show_best = _ovc[0].toggle("🎯 统计回撤档(参考)", value=True, key=f"ovbest_{a}",
                                help="在K线上标出统计回撤档的历史常驻价区（单票多为噪声·非买点）；裁决不背书时自动转灰为'暂非买点'。")
